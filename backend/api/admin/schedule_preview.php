<?php
/**
 * Admin user schedule preview
 * GET /api/admin/schedule_preview.php?user_id=2&date=YYYY-MM-DD
 */
...
if($user_id <= 0) {
    Response::validationError(['user_id' => 'Valid user_id is required']);
}
...
$user_query = "SELECT id, full_name FROM users WHERE id = :user_id AND role = 'user' LIMIT 1";
...
$tasks_query = "SELECT id, title, urgency, estimated_duration, deadline, priority_score, status
                FROM tasks
...
foreach($tasks as $task) {
    ...
    $cursor = $slot_end + (10 * 60);
}

Response::success('Schedule preview generated', [
    'user' => $user,
    'date' => $date,
    'availability' => ['start_time' => $start_time, 'end_time' => $end_time],
    'slots' => $slots
]);
?>
